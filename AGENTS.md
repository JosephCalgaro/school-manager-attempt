...
School Manager is a full-stack web application for managing students, users, and responsibles (guardians) at a school. The codebase is a monorepo with two independent Node.js projects:
...
- The server reads `backend/.env` for `PORT`, `DB_HOST`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`, `DB_PORT`
- Requires a running MySQL instance with a `school_manager` database
...
- `src/app.js` — App setup: registers CORS, JSON body parser, routes, and the `/ping` health-check endpoint.
- `src/database/connection.js` — Creates and exports a mysql2 connection pool (configured via dotenv).
- `src/routes/` — Express routers that map HTTP methods to controller functions. Three route groups:
  - `/auth` (public) — login and profile
  - `/students` (protected) — CRUD for students
  - `/users` (protected) — CRUD for users
- `src/controllers/` — Business logic per entity. Controllers run raw SQL queries against the pool. Student create/update use MySQL transactions to atomically handle the `responsibles` table.
- `src/middlewares/auth.js` — JWT Bearer token verification middleware. Sets `req.userId` and `req.userRole`.
- `src/utils/validator.js` — Shared validation functions (CPF validation with check-digit algorithm, email, date, minor-age detection, payload validators for students and users).
...
- User deletion is a soft-delete (`is_active = 0`), not a hard delete.
- User roles are: `ADMIN`, `SECRETARY`, `TEACHER`. Students are a separate table and do not have user accounts.
- Passwords are hashed with bcryptjs (cost factor 10).
- JWT tokens encode `{ sub: userId, role }` and expire in 10 hours.
...
- `src/App.tsx` — Route definitions. `AppLayout` wraps all authenticated dashboard routes (sidebar + header + `<Outlet />`). `/signin` is outside the layout.
- `src/context/` — React contexts:
  - `AuthProvider` / `AuthContext` — Manages JWT auth state, persists session in `localStorage`, provides `login()`, `logout()`, and `authFetch()` (auto-attaches Bearer token).
...
The frontend calls the backend API at relative paths (e.g., `/auth/login`). In development, configure a Vite proxy or ensure the backend is accessible at the same origin.
...
- Comments in the backend are written in Portuguese (pt-BR).
